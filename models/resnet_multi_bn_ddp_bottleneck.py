...
class batch_norm_multiple(nn.Module):
    def __init__(self, norm, inplanes, bn_names):
        super(batch_norm_multiple, self).__init__()
        self.bn_names = bn_names
        self.bns = nn.ModuleDict({name: nn.BatchNorm2d(inplanes) for name in bn_names})
        
    def forward(self, y):
        x = y[0]
        name_bn = y[1]
        
        # Apply all batch norms
        # torch.distributed.barrier()
        results = {name: bn(x) for name, bn in self.bns.items()}
        # torch.distributed.barrier()
        # Select the correct output based on name_bn
        out = sum(mask * results[name] for name, mask in zip(self.bn_names, self.create_mask(name_bn)))
        
        return out
    
    def create_mask(self, name_bn):
        return [float(name == name_bn) for name in self.bn_names]
...
class proj_head(nn.Module):
    def __init__(self, ch, bn_names=None, twoLayerProj=False):
        ...
        self.fc1 = nn.Linear(ch, 2048)
        self.bn1 = batch_norm_multiple(nn.BatchNorm1d, 2048, bn_names)
        self.fc2 = nn.Linear(2048, ch, bias=False)
